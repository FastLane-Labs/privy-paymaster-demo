        ...
        args.length > 0
          ? args
              .map((a: any) => {
                ...
                return (
                  safeStringify(a).substring(0, 200) + (safeStringify(a).length > 200 ? '...' : '')
                );
              })
              .join(' ')
          : '';
              ...
              ? ' ' +
                args
                  .map((a: any) => {
                    if (typeof a === 'string') return a;
                    return (
                      safeStringify(a).substring(0, 200) +
...
export const formatObject = (obj: any): any => {
  ...
  try {
    ...
    return safeStringify(obj).substring(0, 200) + (safeStringify(obj).length > 200 ? '...' : '');
  } catch (e) {
...
export const formatUserOp = (userOp: any): any => {
  ...
  return {
    ...
    signature: userOp.signature ? `${userOp.signature.substring(0, 10)}...` : 'not set',
    // Handle both v0.6 and v0.7 paymaster formats
    paymasterAndData: userOp.paymasterAndData
      ? userOp.paymasterAndData === '0x'
        ? '0x'
        : `${userOp.paymasterAndData.substring(0, 10)}...`
      : 'not set',
    paymaster: userOp.paymaster || 'not set',
    paymasterData: userOp.paymasterData ? `${userOp.paymasterData.substring(0, 10)}...` : 'not set',
...
export const formatTransaction = (tx: any): any => {
  ...
  return {
    ...
    data: tx.data ? (tx.data === '0x' ? '0x' : `${tx.data.substring(0, 10)}...`) : 'not set',
    ...
    debug: (message: string, ...args: any[]) => {
      if (CURRENT_LOG_LEVEL === 'debug') {
        console.debug(
          '🔍',
          message,
          ...args.map(arg => (typeof arg === 'object' ? formatObject(arg) : arg))
        );
      }
    },
      ...
      if (['debug', 'info'].includes(CURRENT_LOG_LEVEL)) {
        ...
        console.info('ℹ️', message, {
          standard: gasPrice.standard
            ? `${gasPrice.standard.maxFeePerGas?.toString().substring(0, 8)}...`
            : 'N/A',
        });
      }
    },
